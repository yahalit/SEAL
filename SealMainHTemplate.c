...
//TODO: Add period overflow exception 
//TODO: Implement exception and abort processing
...
void GoSeal()
{
    ...
    FuncDescriptor SetupFuncs[N_FUNC_DESCRIPTORS];
    ...
    for (cnt = 0; cnt < N_FUNC_DESCRIPTORS ; cnt++ )
    {
        ...
        case E_Func_ISR: 
            ...
            break;
        case E_Func_Setup: 
            SetupFuncs[nSetupFuncs] = FuncDescriptor[cnt];
            nSetupFuncs += 1;
            break;
        default:
            eol = 1; 
...
void SealLoop()
{
    ...
    if ( SysState.AbortRequest * SysState.AbortiveException == 0)
    {
    ...
    {
        if (SysState.AbortHonored == 0)
        {
            SysState.AbortHonored = 1; 
            if (SysState.AbortiveException)
            {
                for (cnt = 0; cnt < nExceptionFuncs; cnt++)
                {
                    ExceptionFuncs[cnt].func();
                }
            }
            for (cnt = 0; cnt < nAbortFuncs; cnt++)
            {
                AbortFuncs[cnt].func();
            }
        }

function fields = emit_change_function(fid , headerPath, structName, routineName, opts)
...
%
% INPUTS
% fid           : File identifier to write into   
...
for i = 1:numel(decls)
    ...
    for k = 1:numel(parts)
        ...
        if contains(tok,'(') && contains(tok,')'), continue; end % function ptrs – skip
...
% fprintf(fid, "%s %sBG_in ;\n",ctype, ctype);
% fprintf(fid, "%s %sBG_out ;\n",ctype, ctype);
% fprintf(fid, "%s %sRT ;\n",ctype, ctype);
% fprintf(fid, "%s *p%s_Backup ;\n",ctype, ctype);
